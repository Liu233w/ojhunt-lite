"""
Tests for NBUT crawler
"""

import pytest
import pytest_asyncio
import aiohttp
from crawlers.nbut import query
...
async def test_username_with_space(session):
    ...
    # NBUT does not support username with spaces
    with pytest.raises(ValueError, match="does not support username with spaces"):
        await query(session, " " + NOT_EXIST_USERNAME)
...
async def test_valid_user(session):
    ...
    assert "1010" in result["solved_list"]
