...
def test_cancel_query(page: Page):
    def delay_response(route: Route):
        if "api/query" in route.request.url:
            time.sleep(5)
        route.continue_()
    ...
    expect(row.locator(".cancel-btn")).to_be_visible(timeout=2000)
    row.locator(".cancel-btn").click()
...
def test_cancel_shows_immediately(page: Page):
    ...
    expect(row).to_be_visible(timeout=5000)
    row.locator(".query-btn").click()
    expect(row.locator(".cancel-btn")).to_be_visible(timeout=1000)
    row.locator(".cancel-btn").click()
