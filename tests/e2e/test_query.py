...
def test_query_success(page: Page):
    ...
    expect(row.locator("td:nth-child(3)")).not_to_have_text("-", timeout=30000)
...
def test_query_user_not_found(page: Page):
    page.goto(BASE_URL)
    page.select_option("select[x-model='selectedCrawler']", "codeforces")
    page.fill("input[placeholder='Username']", "nonexistentuser12345xyz")
    page.click('button:has-text("Add")')
    row = page.locator("tbody.result-row").filter(has_text="nonexistentuser12345xyz")
    expect(row).to_be_visible(timeout=5000)
    row.locator("button.query-btn").click()
