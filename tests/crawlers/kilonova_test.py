"""
Tests for Kilonova crawler
"""

import pytest
from ojhunt.crawlers.kilonova import __crawler_meta__, query
...
async def test_username_with_space(session):
    """Test that username with space raises ValueError"""
    with pytest.raises(ValueError, match="Please enter username"):
        await query(session, " ")


@pytest.mark.asyncio
async def test_empty_username(session):
